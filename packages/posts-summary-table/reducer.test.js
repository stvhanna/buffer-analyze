...
describe('reducer', () => {
  ...
  it('has no metrics upon receiving FETCH_START', () => {
    const metrics = reducer(undefined, {
      type: `posts_summary_${asyncDataFetchActions.FETCH_START}`,
    }).metrics;
    expect(metrics).toEqual([]);
  });
  it('on fetch_success stops loading', () => {
    const state = reducer(undefined, {
      type: `posts_summary_${asyncDataFetchActions.FETCH_SUCCESS}`,
    });
